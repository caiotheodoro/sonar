...
W6.1-clean/dry/v3 (superseded) ~$2.8 + probes/canaries ~$0.5 ≈ **$7.6
Monid** to date. OpenAI ≈ **$0.55** total. The $3/day workspace budget
cap (control `01M1GB8M12…`) was the evening's blocker; the operator raised
it. Remaining `$` work: W7.2 narration (~$0.03), W8.2 rehearsal (~$0.8).
...
| W7.2 narration TTS | one ElevenLabs run through `voice/tts` | 0.10 | not run | next — needs `video/src/data/narration.json` written from the frozen demo |
