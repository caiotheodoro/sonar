...
| OQ-5 | Trustpilot and G2 native id, rating, timestamp and author field names | Mention fields typed as above; adapter fills `native_id` from whatever unique review id the payload exposes, with named fallbacks | The first live Trustpilot and G2 runs (W5.5): `docs/monid/inspect/*.json` carries the request schema only, not the review-object shape |
